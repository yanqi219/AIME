...
final_table <- function(wd_annotation, wd_feature, data_feature_pls, data_feature_mummichogInput, wd_pathway,
                        data_verified, whole_list=FALSE){
  if(whole_list == FALSE){
    ####################
    # annotation
    ####################
    ...
    mummichog_pathway_complete$mz <- round(mummichog_pathway_complete$mz, 4)
    # merge only confidence score >= 2
    
    # KEGG first
    annotation_KEGG_highconf <- annotation_KEGG[which(annotation_KEGG$Confidence >= 1),]
    feature_w_annotation <- merge(feature_PLSDA[,c(1:4)], annotation_KEGG_highconf[,c(1,2,5,6,10,13)], by = c("mz", "time"), all = T)
    ...
    feature_w_annotation <- merge(feature_w_annotation[,c(1:4)], annotation_HMDB_highconf[,c(1,2,5,6,10,13)], by = c("mz", "time"), all = T)
    feature_w_annotation <- feature_w_annotation[-which(is.na(feature_w_annotation$vip)),]
    flag <- which(!is.na(feature_w_annotation$chemical_ID))
    ...
    mummichog_pathway_complete <- mummichog_pathway_complete[,-1]
    merge_all <- merge(feature_w_annotation, mummichog_pathway_complete, by = c("mz", "time"), all = T)
    ...
    # remove those non-significant features picked by mummichog
    merge_all <- merge_all[-which(is.na(merge_all$vip)),]
    merge_all <- merge_all[order(merge_all$mz),]
...
write.table(HILIC_autism, file = "C:/Users/QiYan/Dropbox/AIME/Results/Autism/HILIC_final_table_server.txt", sep = "\t", row.names = F,quote = F)
write.table(C18_autism, file = "C:/Users/QiYan/Dropbox/AIME/Results/Autism/C18_final_table_server.txt", sep = "\t", row.names = F,quote = F)
...
write.table(HILIC_autism, file = "C:/Users/QiYan/Dropbox/AIME/Results/Autism/HILIC_whole_table_server.txt", sep = "\t", row.names = F,quote = F)
write.table(C18_autism, file = "C:/Users/QiYan/Dropbox/AIME/Results/Autism/C18_whole_table_server.txt", sep = "\t", row.names = F,quote = F)
